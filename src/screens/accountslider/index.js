...
// import this.props.movie from './data'
...
export default class MovieHome extends Component {
  ...
  render() {
    ...
    const noOfSlides = Math.ceil(this.props.movies.length / cardPerSlide)
    return (
      <View style={styles.container}>
        {/* <View style={styles.navBar}>
          <Text style={styles.navBarTitle}>this.props.movie</Text>
        </View> */}
        <View style={{flexDirection:'row',alignItems:'center',backgroundColor:"#000000"}}>
        <TouchableOpacity style={[ !isPrev && styles.disable,{backgroundColor:"#000000"}]} onPress={this.goToPrev} disabled={!isPrev}>
            <Image source={AppImages.prev}/>
          </TouchableOpacity>
        ...
        >
          {[...Array(noOfSlides)].map((e, i) => {
            ...
            const endPosition = (startIndex * 4)
            return <CarouselSlide key={i} cards={this.props.movies.slice(startPosition, endPosition)} />
          })}
        </ScrollView>
        <TouchableOpacity style={[ !isNext && styles.disable,{backgroundColor:"#000000",}]} onPress={this.goToNext} disabled={!isNext}>
